...
- 🎲 **智能伪随机模式选择**：启用后每次刷新随机切换模式，保证连续3次刷新体验不同模式
...
popup:
  ...
  randomMode: false              # 是否启用随机模式选择，启用后每次刷新页面会随机选择一种显示模式
...
2. **智能随机模式**：
   - `randomMode`：启用后，每次用户刷新网页时会智能选择显示模式
   - **伪随机算法**：使用浏览器 localStorage 记录历史，确保连续3次刷新显示不同的模式
   - **工作原理**：
     - 系统记录用户最近2次使用的模式
     - 下次刷新时，从未使用过的模式中随机选择
     - 保证每个用户连续3次刷新都能体验到不同的显示效果
     - 当3种模式都体验过后，自动重置历史，重新开始循环
   - **用户独立**：每个浏览器独立维护历史记录，不同用户互不影响
...
- 1.4.1：新增智能伪随机模式选择功能，使用 localStorage 实现历史记录管理，保证用户连续3次刷新体验不同模式；完善配置系统，支持通过 randomMode 参数控制随机模式开关
